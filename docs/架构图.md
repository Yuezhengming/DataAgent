...
graph TB
    subgraph "混合检索引擎"
        A[HybridRetrievalStrategy]
        A1[AbstractHybridRetrievalStrategy]
        A2[DefaultHybridRetrievalStrategy]
        A3[ElasticsearchHybridRetrievalStrategy]
        A --> A1
        A1 --> A2
        A1 --> A3
    end
    ...
    subgraph "检索执行层"
        B1[向量检索<br/>Vector Search]
        B2[关键词检索<br/>Keyword Search]
        B3[并行执行器<br/>ExecutorService]
        B3 --> B1
        B3 --> B2
    end
    ...
    subgraph "融合策略层"
        C[FusionStrategy]
        C1[RrfFusionStrategy<br/>倒数排名融合]
        C2[WeightedAverageStrategy<br/>加权平均融合]
        C --> C1
        C --> C2
    end
    
    subgraph "向量存储层"
        D1[(SimpleVectorStore<br/>内存向量库)]
        D2[(Elasticsearch<br/>分布式搜索)]
        D3[(Milvus<br/>专业向量库)]
    end
