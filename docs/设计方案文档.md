...
│  - Graph 工作流引擎  - NL2SQL 核心逻辑  - 混合检索系统      │
│  - Python 代码执行  - 语义一致性校验  - 融合策略引擎        │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                        数据层 (Data Layer)                    │
│  - MySQL (业务数据)  - 向量数据库 (Schema 索引)              │
│  - Elasticsearch (混合检索)  - 业务数据源 (多数据源支持)     │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
│                      AI 服务层 (AI Services)                  │
...
| **核心引擎层** | NL2SQL 逻辑、工作流编排、混合检索、AI 调用 | Spring AI, Graph Engine, Hybrid Retrieval |
...
| **向量检索** | SimpleVectorStore / Milvus / Elasticsearch | - |
| **混合检索** | 向量+关键词检索 | RRF / 加权平均融合 |
...
混合检索系统是 DataAgent 的重要增强功能，通过结合向量检索和关键词检索，显著提升了 Schema 召回的准确性和覆盖度。

#### 5.5.1 系统架构

**核心组件**:
- `HybridRetrievalStrategy` - 混合检索策略接口
- `AbstractHybridRetrievalStrategy` - 抽象基类，实现通用逻辑
- `DefaultHybridRetrievalStrategy` - 默认实现（仅向量检索）
- `ElasticsearchHybridRetrievalStrategy` - Elasticsearch 实现

**融合策略**:
- `FusionStrategy` - 融合策略接口
- `RrfFusionStrategy` - 倒数排名融合（RRF）
- `WeightedAverageStrategy` - 加权平均融合

#### 5.5.2 工作流程

1. **并行检索**：同时执行向量检索和关键词检索
2. **结果融合**：使用配置的融合策略合并结果
3. **排序输出**：返回融合后的 TopK 结果

**执行流程**:
```
用户查询 → 关键词提取 → 并行检索
                        ↓
向量检索 ←→ ExecutorService ←→ 关键词检索
   ↓                              ↓
向量结果 → FusionStrategy ← 关键词结果
                ↓
           融合后的结果
```
...
spring:
  ai:
    vectorstore:
      type: simple          # 使用 DefaultHybridRetrievalStrategy
      # type: elasticsearch # 使用 ElasticsearchHybridRetrievalStrategy
```
...
spring:
  ai:
    alibaba:
      data-agent:
        fusion-strategy: rrf      # 或 weighted
        elasticsearch-min-score: 0.5
```

#### 5.5.4 Elasticsearch 集成
...
```json
{
  "query": {
    "bool": {
      ...
      "filter": [
        {"term": {"metadata.agentId": "1"}},
        {"term": {"metadata.vectorType": "table"}}
      ]
    }
  },
  "size": 20,
...
- **异步执行**：向量检索和关键词检索并行执行
- **结果缓存**：缓存常用查询的检索结果
- **TopK 优化**：检索时使用 2×TopK，融合后返回 TopK
